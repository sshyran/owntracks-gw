...
/**
 * Data container of stack type for position strings, which can be accessed by
 * multiple threads without the risk of collisions. Keeps track of the last GPS
 * position (valid or not),	an indicator of the validity of the last GPS
 * position and last GPS VALID string.
 *
...
public class DataStore extends Stack implements GlobCost {
     ...
     */
    /**
     * Synchronized method to replace object at the top of the stack, if stack
     * is empty simply insert a new object at the top. If object is valid,
     * update validity indicator and replace the last valid GPS position with
     * current position.
     *
     * @param	item	object to insert
     * @param isValid	'true' indicates that inserted object is valid, 'false'
     * means not valid
    ...
    /**
     * Synchronized method to read lasta valid GPS position (valid or not) saved
     * in DataStore, without get from stack. Return 'null' if you have not
     * already done the first FIX.
     *
    ...
    /**
     * Synchronized method to read last valid GPS position saved in DataStore,
     * ONLY if is valid and without get from stack. Return 'null' if last GPS
     * position is not valid or if you have not already done the first FIX.
     *
     * @return object at the top of the stack or 'null'
     *
     */
    ...
    /**
     * Synchronized method to get last valid GPS position saved in DataStore.
     * Return 'null' if you have not already done the first FIX.
     *
     * @return	GPRMC string corresponding to last valid GPS position or 'null'
     * if you have not already done the first FIX
     *
     */
    ...
    /**
     * Synchronized method to read an item from the top of the stack without
     * remove it
     *
